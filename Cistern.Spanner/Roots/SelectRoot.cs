    ...
    : IStreamNode<TInput, TOutput>
{
    ...
    {
        public static TResult Invoke<TFinal, TResult, TProcessStream, TContext>(in TProcessStream stream, in ReadOnlySpan<TInput> span, ref StreamState<TFinal> state, in Func<TInput, TOutput> selector)
            where TProcessStream : struct, IProcessStream<TOutput, TFinal, TResult>
        {
            var localCopy = stream;
            Iterator.Select<TInput, TFinal, TOutput, TProcessStream, TContext>(ref state, in span, ref localCopy, selector);
            return localCopy.GetResult(ref state);
        }

        TResult LargeStackAllocator.IAfterAllocation<TInput, TOutput, Func<TInput, TOutput>>.Execute<TFinal, TResult, TProcessStream, TContext>(in TProcessStream stream, in ReadOnlySpan<TInput> span, ref StreamState<TFinal> state, in Func<TInput, TOutput> selector) =>
            Invoke<TFinal, TResult, TProcessStream, TContext>(in stream, in span, ref state, selector);
    }
    ...
    TResult IStreamNode<TInput, TOutput>.Execute<TFinal, TResult, TProcessStream, TContext>(in TProcessStream processStream, in ReadOnlySpan<TInput> span, int? stackAllocationCount)
    {
        if (!stackAllocationCount.HasValue || stackAllocationCount <= 0)
            ...
            return LargeStackAllocator.Execute<TInput, TOutput, TFinal, TResult, TProcessStream, Func<TInput, TOutput>, Execute, TContext>(stackAllocationCount.Value, in span, in processStream, Selector);
    }
        ...
        where TProcessStream : struct, IProcessStream<TOutput, TFinal, TResult>
    {
        ...
        return Execute.Invoke<TFinal, TResult, TProcessStream, TContext>(in processStream, in span, ref state, Selector);
    }
